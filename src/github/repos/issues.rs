...
use crate::model::{
    Token,
    prs::{self, PullRequestId, find_pr, reviews},
};
...
fn issues_router() -> Router<St> {
    Router::new()
        ...
        .route("/{number}", get(get_issue).patch(update_issue))
        .route("/{number}/labels", get(get_issue_labels).put(replace_issue_labels).post(add_issue_labels).delete(delete_issue_labels))
...
async fn get_pull_request(
    ...
    let Some(pr_id) = prs::find_by_number(&tx, (&owner, &name), pull_number)
    else {
...
async fn get_pull_request_commits(
    ...
    let mut pr_commits = {
        ...
        for i in 0.. {
            ...
            commits.push(
                super::Bundle(&st, &tx, &owner, &name, &oid, &commit).into(),
            );
        }
        commits
    };
...
async fn create_pull_request(
    ...
    fn validation_failed<'a>(
        ...
        Error::unprocessable("Validation Failed", es).into_response(
            "pulls",
            "pulls",
    ...
    let (_base, head) = match (base, head) {
        (None, None) => {
            return Err(
    ...
    let issue_id = match req._source {
        ...
        PullRequestSource::FromIssue { issue } => {
            let res = match prs::find_issue_id(&tx, &owner, &name, issue)
                .map(|issue_id| (issue_id, prs::from_issue(&tx, issue_id)))
            {
                Some((id, None)) => Ok(id),
                Some((_, Some(_))) => Err(
                    "The specified issue is already attached to a pull request.",
                ),
                None => Err("The specified issue does not exist."),
            };
            match res {
                Ok(issue) => issue,
                Err(err) => {
    ...
    // rules don't apply to magic ref
    let _ = crate::model::git::refs::create(
        &tx,
        &repo,
...
async fn update_pull_request(
    ...
    let Some(pr) = find_pr(&tx, &owner, &name, number) else {
        return Err(Error::NOT_FOUND
            .into_response("pulls", "pulls", "update-a-pull-request")
            .into_response());
    };
    ...
    prs::update(&tx, pr.id)
        ...
        .maybe_body(body.as_ref().map(|b| b.as_deref()))
        .maybe_state(state)
        .call();
                ...
                webhooks::WebhookEvent::PullRequest(webhooks::PullRequest {
                    action: webhooks::PullRequestAction::Edited {
                        changes: webhooks::PrEdition {
                            ...
                            body: body.as_ref().map(|_| {
                                webhooks::Change::from(
                                    pr.issue.body.clone().unwrap_or_default(),
                                )
                            }),
...
async fn create_review(
    ...
    let Some(pr) = prs::find_by_number(&tx, repo.id, pr) else {
        return Err(Error::NOT_FOUND.into_response(
            "pulls",
            "reviews",
            "create-a-review-for-a-pull-request",
...
async fn list_reviews(
    ...
    let Some(pr) = prs::find_by_number(&tx, (&owner, &name), pr) else {
        return Err(Error::NOT_FOUND.into_response(
            "pulls",
            "reviews",
...
async fn list_review_comments(
    ...
    let Some(pr_id) = prs::find_by_number(&tx, (&owner, &name), pr_number)
    else {
        return Err(Error::NOT_FOUND.into_response(
            "pulls",
            "reviews",
...
async fn get_issue(
    ...
    let response = to_issue_response(&tx, &st, &owner, &name, issue);
...
async fn update_issue(
    ...
    // let user = crate::github::auth_to_user(&tx, auth).unwrap();
    let Some(issue) = prs::find_issue_id(&tx, &owner, &name, number)
    ...
    let title = update
        .title
        .inspect(|t| assert!(!t.is_empty(), "title can't be updated to empty"))
        .filter(|t| t != &issue.title);
    let body = update
        .body
        .map(|b| b.filter(|s| !s.is_empty()))
        .filter(|b| b != &issue.body);
    let state = update
        .state
        .map(|s| match s {
            IssueState::Open => prs::State::Open,
    ...
    if title.is_none() && body.is_none() && state.is_none() {
        return Ok(Json(to_issue_response(&tx, &st, &owner, &name, issue)));
    }

    prs::update(&tx, issue.id)
        .maybe_title(title.as_deref())
        .maybe_body(body.as_ref().map(|b| b.as_deref()))
        .maybe_state(state)
        .call();
...
enum _Labels {
    ...
    //LabelsNamed { labels: Vec<_Label> },
}
impl From<_Labels> for Labels {
    fn from(value: _Labels) -> Self {
        match value {
            _Labels::Strings(labels) => Labels {
                labels,
                structured: false,
            },
            _Labels::Named(ls) => Labels {
                labels: ls.into_iter().map(|l| l.name).collect(),
                structured: false,
            },
            _Labels::Labels { labels } => Labels {
                labels,
                structured: true,
            },
        }
...
async fn add_issue_labels(
    ...
    if labels.labels.is_empty() && labels.structured {
        return Err(Error::Unprocessable("argle bargle".into(), &[])
            .into_response("issues", "labels", "add-labels-to-an-issue"));
    }
...
async fn delete_issue_labels(
    ...
    for label_id in prs::get_labels(&tx, issue, |l| l.id) {
        prs::remove_label(&tx, issue, label_id);
    }
    tx.commit();
...
async fn delete_issue_label(
    State(st): State<St>,
    tx: Token<Write>,
    Path((owner, name, issue_number, label)): Path<(
        String,
        String,
        usize,
        String,
...
async fn create_issue_comment(
    ...
    send_hook(&tx, repo, &st, HookEvent::IssueComment, &user, || {
        ...
        .into_response(&st.root, &owner, &name);
        issue_response.pull_request = prs::from_issue(&tx, issue.id)
...
async fn update_issue_comment(
    ...
    tx: Token<Write>,
    Path((owner, name, comment_id)): Path<(String, String, i64)>,
    Json(IssueCommentUpdate { body }): Json<IssueCommentUpdate>,
) -> Result<Json<IssueCommentResponse>, GHError<'static>> {
    let Some(user) = crate::github::auth_to_user(&tx, auth) else {
        return Err(Error::Unauthenticated("").into_response(
            "issues",
