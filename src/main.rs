...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "otlp")]
    let (handler, _trace_guard) = {
        use axum_tracing_opentelemetry::middleware::{
            OtelAxumLayer, OtelInResponseLayer,
        };
        let trace_guard = init_tracing_opentelemetry::tracing_subscriber_ext::init_subscribers()?;
        (
            handler
...
async fn load_users(p: &Path) -> Result<(), Box<dyn std::error::Error>> {
