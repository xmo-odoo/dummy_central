...
def test_empty_repo(request, org):
    ...
    with pytest.raises(GithubException) as ghe:
        r.create_git_blob('blobish', "utf-8")
...
def test_create_blob(repo, session):
    data = "blobish"
    ...
    # NOTE: use requests because pygithub requires an encoding: str
    r = session.post(f'{repo.url}/git/blobs', json={"content": contents})
    assert r.status_code == 201
    assert r.json()['sha'] == b0.sha
