...
def set_file(repo, ref, *, message, name, content):
    ...
    t = repo.create_git_tree(
        [item(name, '100644', 'blob', content)], base_tree=previous.tree
    )
...
def test_synchronize(repo, config, endpoint, request):
    ...
    pr = repo.create_pull(repo.default_branch, "pr", title="test")
    payload = pr_payload(get_hook())
    assert payload['action'] == 'opened'
    update = set_file(
        repo, pr_branch, message='an other', name='foo', content='blorp again'
    )
    payload = pr_payload(get_hook())
